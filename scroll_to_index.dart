...
// Author: Jerry Chen
///https://pub.dev/packages/scroll_to_index/example
/// 하이라이트 영역을 에니메이션으로 변경
...
import '../../widget/animated/chain_tweens.dart';
import 'util.dart';
...
mixin AutoScrollControllerMixin on ScrollController
  ...
  Future highlightColor(int index,
      {bool cancelExistHighlights: true,
        Duration highlightDuration: _highlightDuration,
        bool animated: true}) async {
    print("check highlightColor");
    final tag = tagMap[index];

    return tag == null
        ? null
        : await tag.highlightColor(
        cancelExisting: cancelExistHighlights,
        highlightDuration: highlightDuration,
        animated: animated);
  }
...
class AutoScrollTagState<W extends AutoScrollTag> extends State<W>
  ...
  bool highLightType = true;
  List<Offset> values = const [Offset(0, 0), Offset(0, -15),Offset(0, 15)];
  ...
  Widget build(BuildContext context) {
    ...
    return widget.builder?.call(context, animation)
          ?? buildHighlightTransition(context: context, highlight: animation, child: widget.child!,
            translationXAnim:_translationXAnim, translationYAnim:_translationYAnim,highLightType:highLightType,
  ...
  Future highlightAnimation(
    ...
    if (_controller == null) {
      _controller = new AnimationController(vsync: this,upperBound:2);
      _highlights[this] = _controller;
      highLightType = true;
      _translationXAnim = chainTweens(
        values.map((it) => it.dx).toList(),
      ).animate(
...
Widget buildHighlightTransition({required BuildContext context, required Animation<double> highlight, 
  required Widget child, Color? background, Color? highlightColor, Animation<double>? translationXAnim, Animation<double>? translationYAnim, required bool highLightType}) {
  return highLightType?Transform.translate(
    offset:(translationXAnim==null&&translationYAnim==null)?Offset(0,0): Offset(translationXAnim!.value, translationYAnim!.value),
    child: child,
  ):
